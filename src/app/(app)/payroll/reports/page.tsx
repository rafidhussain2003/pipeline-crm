...
// Phase 21 — reports are PLACEHOLDERS only (architecture registered in
// lib/payroll; implementations are a future phase).
export default function PayrollReportsPage() {
  const [reports, setReports] = useState<{ key: string; label: string }[]>([]);

  useEffect(() => {
    fetch("/api/payroll/settings").then(async (r) => { if (r.ok) setReports((await r.json()).reports || []); });
  }, []);

  return (
    <div className="p-6 max-w-3xl">
      <PageHeader title="Reports" subtitle="Payroll reporting is coming in a future update." />
