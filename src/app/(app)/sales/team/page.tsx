...
// Sales Ledger → Backend Agents. The admin adds backend employees (name +
// email + temporary password); each one logs in, lands on the Sales Ledger,
// sees EVERY sale and works them (edit statuses/details). They never see the
// CRM, Commercial Sales or any other module, and they cannot delete/restore/
// export or manage this roster. Admin-only (the API 403s everyone else).

type Employee = { id: string; name: string; email: string; active: boolean; createdAt: string };

export default function BackendAgentsPage() {
  ...
  async function add(e: React.FormEvent) {
    ...
    if (!res.ok) {
      setMessage({ kind: "error", text: data.error || "Could not add the backend agent." });
      return;
    }
  ...
  if (forbidden) {
    return <div className="p-6 text-sm text-slate-500">Only a company admin can manage Backend Agents.</div>;
  }

  return (
    <div className="p-6 max-w-3xl">
      <div className="mb-5">
        <h1 className="text-xl font-semibold text-slate-900">Backend Agents</h1>
        <p className="text-sm text-slate-500 mt-0.5">
          Backend employees who log in and work the whole Sales Ledger — every agent&rsquo;s sales, statuses and details.
          They only ever see the Sales Ledger, never the CRM or Commercial Sales.
        </p>
      </div>
      {message && (
          ...
          <form onSubmit={add} className="space-y-3">
            <div className="grid sm:grid-cols-3 gap-3">
              <div>
                ...
                <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required className="w-full rounded-md border border-slate-200 px-3 py-2 text-sm" placeholder="e.g. Adil Hussain" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1">Email</label>
                <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required className="w-full rounded-md border border-slate-200 px-3 py-2 text-sm" placeholder="backend@company.com" />
              </div>
              <div>
            ...
            <div className="flex items-center gap-2">
              <button type="submit" disabled={saving} className="text-sm font-semibold text-white bg-slate-900 hover:bg-slate-800 rounded-md px-3 py-2 disabled:opacity-40">
                {saving ? "Adding…" : "Add backend agent"}
              </button>
      ...
      <div className="bg-white border border-slate-200 rounded-lg divide-y divide-slate-100">
        ...
        {employees && employees.length === 0 && <div className="p-4 text-sm text-slate-400">No backend agents yet.</div>}
        {employees?.map((e) => (
...
function EmployeeRow({ employee, onChanged, onMessage }: { employee: Employee; onChanged: () => void; onMessage: (m: { kind: "ok" | "error"; text: string }) => void }) {
  ...
  async function patch(body: Record<string, unknown>, okText: string) {
    setBusy(true);
    const res = await fetch(`/api/sales/team/${employee.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    setBusy(false);
