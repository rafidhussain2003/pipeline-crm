...
// Edit a Backend Agent: activate/deactivate, or reset their temporary
// password. Admin-only and tenant-scoped — the target must be a backend_agent
// of THIS company, or it is a 404 (never leak that a user exists elsewhere).
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [target] = await db
    ...
    .from(users)
    .where(and(eq(users.id, id), eq(users.companyId, session.companyId), eq(users.role, "backend_agent"), isNull(users.deletedAt)))
    .limit(1);
  if (!target) return NextResponse.json({ error: "Backend agent not found." }, { status: 404 });
  ...
  await recordAudit({
    ...
    action: "sales.backend_agent_updated",
    entityType: "user",
    entityId: id,
